...
export default function EditUsahaPage() {
    ...
    async function checkAuthAndLoadData() {
      ...
      if (id) {
        ...
        if (result.success && result.data) {
          setFormData({
            kodeProv: result.data.kodeProv || 14,
            kodeKab: result.data.kodeKab || 5,
            kodeKec: result.data.kodeKec?.toString() || '',
            kodeDesa: result.data.kodeDesa?.toString() || '',
            kodeSls: result.data.kodeSls?.toString() || '',
            idRt: result.data.idRt?.toString() || '',
            idArt: result.data.idArt?.toString() || '',
            r421Desk: result.data.r421Desk || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        setSuccessMessage('Data usaha berhasil diperbarui!')
        // Auto redirect setelah 2 detik
        setTimeout(() => {
          router.push('/admin')
        }, 2000)
      } else {
        setErrorMessage(result.error || 'Gagal memperbarui data')
      }
    } catch (err) {
        ...
        <section className="min-w-0 flex-1 px-3 py-4 md:px-5 bg-gray-50">
          <div className="max-w-4xl mx-auto">
            <div className="mb-8">
              ...
              <h1 className="text-3xl font-bold text-gray-800 mb-2">Edit Data Usaha</h1>
              <p className="text-gray-600">Ubah data usaha yang sudah ada</p>
            </div>
